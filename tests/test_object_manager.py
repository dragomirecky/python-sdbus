...
class TestObjectManager(IsolatedDbusTestCase):
    async def test_object_manager(self) -> None:
        ...
        self.assertEqual(await object_manager_connection.get_hello_world(), HELLO_WORLD)
    ...
    async def test_parse_interfaces_added_removed(self) -> None:
        ...
        with self.subTest("Parse added class"):
            path, python_class, python_properties = parse_interfaces_added(
                ManagedTwoInterface, caught_added
            )
        ...
        with self.subTest("Parse added iterable"):
            path, python_class, python_properties = parse_interfaces_added(
                (ManagedInterface, ManagedTwoInterface), caught_added
            )
        ...
        get_managed_data = await object_manager_connection.get_managed_objects()
        with self.subTest("Parse get managed objects class"):
            managed_dict = parse_get_managed_objects(
                ManagedTwoInterface,
                get_managed_data,
            )

            self.assertIn(MANAGED_PATH, managed_dict)
            managed_class, managed_properties = managed_dict[MANAGED_PATH]
        ...
        with self.subTest("Parse removed class"):
            path, python_class = parse_interfaces_removed(
                ManagedTwoInterface,
