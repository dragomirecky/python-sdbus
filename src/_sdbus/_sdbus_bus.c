...
#endif
    SdBusMessageObject * new_message_object CLEANUP_SD_BUS_MESSAGE = (SdBusMessageObject *)CALL_PYTHON_AND_CHECK(SD_BUS_PY_CLASS_DUNDER_NEW(SdBusMessage_class));
...
int _SdBus_signal_callback(sd_bus_message * m, void * userdata, sd_bus_error * Py_UNUSED(ret_error)) {
    ...
    SdBusMessageObject * new_message_object CLEANUP_SD_BUS_MESSAGE = (SdBusMessageObject *)CALL_PYTHON_CHECK_RETURN_NEG1(SD_BUS_PY_CLASS_DUNDER_NEW(SdBusMessage_class));
...
int _SdBus_match_signal_instant_callback(sd_bus_message * m, void * userdata, sd_bus_error * Py_UNUSED(ret_error)) {
    ...
    if (!sd_bus_message_is_method_error(m, NULL)) {
        SdBusSlotObject * slot_object CLEANUP_SD_BUS_SLOT = (SdBusSlotObject *)CALL_PYTHON_CHECK_RETURN_NEG1(PyObject_GetAttrString(new_future, "_sd_bus_slot"));
...
static PyObject * SdBus_emit_properties_changed(SdBusObject * self, PyObject * args) {
    // signature: path: str, interface, *properties: str
    Py_ssize_t len = PyTuple_Size(args);
    char ** c_args CLEANUP_STR_MALLOC_PTR = malloc(len * sizeof(char *) + 1);
    ...
    c_args[len] = NULL;
    CALL_SD_BUS_AND_CHECK(sd_bus_emit_properties_changed_strv(self->sd_bus_ref, c_args[0], c_args[1], &c_args[2]));
    return Py_None;
}
...
static PyMethodDef SdBus_methods[] = {
    ...
    { "emit_properties_changed", (SD_BUS_PY_FUNC_TYPE)SdBus_emit_properties_changed, METH_VARARGS, PyDoc_STR("Emit signal that properties were changed.") },
